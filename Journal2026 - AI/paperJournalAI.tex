...
%
Depending on the system requirements, these objectives are specified using different winning conditions on infinite paths. These are broadly categorized into qualitative and quantitative conditions. Qualitative conditions focus on the structural or topological properties of infinite paths; classic examples include Büchi conditions (visiting designated states infinitely often), Rabin conditions, and parity conditions (evaluating the parity of the maximum priority seen infinitely often) \cite{Chatterjee:2012,Erich:2008}. Quantitative conditions, on the other hand, incorporate resource constraints or performance metrics, such as energy conditions (keeping a running resource counter above zero) and mean-payoff conditions (optimizing the long-run average reward)\cite{ChatterjeeHJ05,Chaloupka:2011}.
%
...
A Rabin condition is parameterized by a set of pairs $P = \{(L_1, U_1), \dots, (L_m, U_m)\}$, where $L_i, U_i \subseteq V$ for $i \in \{1, \dots, m\}$. Player~$\even$ wins the Rabin objective if there exists at least one index $i$ where the play visits $L_i$ infinitely often but $U_i$ only finitely often:
\[
\exists i \in \{1, \dots, m\} \text{ such that } \text{Inf}(\pi) \cap L_i \neq \emptyset \text{ and } \text{Inf}(\pi) \cap U_i = \emptyset
\]
The Streett condition is the deterministic complement of the Rabin condition. It requires that for every index $i$, if the set $L_i$ is visited infinitely often, then the set $U_i$ must also be visited infinitely often:
\[
\forall i \in \{1, \dots, m\}, \quad \text{Inf}(\pi) \cap L_i \neq \emptyset \implies \text{Inf}(\pi) \cap U_i \neq \emptyset
\]
...
\[
\mu(\pi) = \liminf_{n \to \infty} \frac{1}{n} \sum_{i=0}^{n-1} \Xi(e_i)
\]
...
To demonstrate the structural and operational mechanics of our generalized CSP formulation, consider the 2-player game arena illustrated in Figure~\ref{fig:ex10}. The arena consists of four vertices $V = \{v_0, v_1, v_2, v_3\}$, which are partitioned into $V_{\subeven} = \{v_0, v_2\}$ (represented as circles) and $V_{\subodd} = \{v_1, v_3\}$ (represented as squares). The directed edge set is defined as $E = \{e_0, e_1, e_2, e_3, e_4, e_5, e_6 \}$. We evaluate a Büchi objective from the perspective of Player~$\even$ ($\player = \even$), where the target accepting set is $F = \{v_3\}$, and the initial vertex is $v_0$.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item If the solver branches and tests $\mathcal{E}_0 = \top$, forward propagation ($\mathcal{C}_E$) activates vertex $v_1$ ($\mathcal{V}_1 = \top$). Because vertex $v_1$ belongs to Player~$\odd$, its outgoing edge is forced active ($\mathcal{E}_2 = \top$), closing a localized cycle on vertex $v_1$ (See Figure \ref{fig:ex11}). The global constraint evaluates $f^\mathcal{B}(v_1, v_1) = \{v_1\} \cap \{v_3\} \neq \emptyset$, which returns $\bot$. The propagator immediately rejects this branch, forcing the solver to backtrack.
    ...
    \]
    Consequently, both $\mathcal{E}_5$ and $\mathcal{E}_6$ are set to $\top$ (See Figure \ref{fig:ex12}).
\end{enumerate}
...
In the example shown in Figure~\ref{fig:ex20}, the goal is to find a winning strategy for the Player \even to hold parity condition and a positive energy condition simultaneously ($f = f^{\omega} \otimes f^{\eta}$) starting at $v_0$.
